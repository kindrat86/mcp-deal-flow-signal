  ...
  try {
    switch (name) {
      ...
      case "share_result": {
        ...
        if (wantTelegram) {
          const body = `${summary}\n\nFrom the GitDealFlow MCP server — free, install with: ${installCommand}\n\nThe methodology is published on SSRN with a 219-startup panel: ${ssrn}\nProduct: ${site}\n\nGitDealFlow tracks 350+ startups and ranks them by commit velocity acceleration, weekly. The pattern preceded confirmed fundraises by 21 to 47 days.`.slice(0, 995);
          posts.push({
            network: "telegram",
            body,
            charCount: body.length,
